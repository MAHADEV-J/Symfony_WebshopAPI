...
Het zou wellicht beter in overeenstemming zijn met de rURL routes om een aparte `CartController` te maken met een functie `addProduct()` of `update()`. Maar ik heb ervoor gekozen om dit niet te doen, omdat je het toevoegen van een product aan een winkelwagen ook kunt zien als iets wat je doet met het product en niet met de winkelwagen. Bovendien is de winkelwagen iets dat alleen bestaat in het geheugen en de naam `update()` zou ten onrechte de indruk geven dat het om een CRUD-functie gaat die de database bijwerkt.
